...
;(function(window) {
	 ...
	 * GridLoaderFx obj.
	 */
	function GridLoaderFx(el, items) {
		this.el = el;
		this.items = items || this.el.querySelectorAll('.grid__item > .grid__link');
	}
	...
	function init(items) {
		items = items||null;
		var grids = [].slice.call(document.querySelectorAll('.grid')), masonry = [], loaders = [], _effects = [
			'Hapi',
			'Amun',
			'Kek',
			'Isis',
	...
	$window.on("scroll", function() {
		...
		scrollTimer = setTimeout(function() {
			var last = $('.grid').offset().top + $('.grid').outerHeight(),
				scrollTop = $window.scrollTop() + $window.height();

			if(!ajaxLoading && scrollTop > last) {
				ajaxLoading = true;
